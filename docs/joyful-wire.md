...
This document describes the shared wire package: `@slopus/joyful-wire`.

## Why this package exists

Before `joyful-wire`, wire-level message and session-protocol schemas were duplicated across packages (CLI, app, server, and agent). That caused drift risk and made protocol evolution harder.

`@slopus/joyful-wire` centralizes those shared schemas and types so all clients and services agree on the same wire contract.

## Package identity

- npm name: `@slopus/joyful-wire`
- workspace path: `packages/joyful-wire`
...
Shared from `@slopus/joyful-wire`:
...
### CLI (`packages/joyful-cli`)

- Session protocol imports now reference `@slopus/joyful-wire` directly.
- `src/sessionProtocol/types.ts` now re-exports from `@slopus/joyful-wire` as compatibility shim.
- API wire schemas in `src/api/types.ts` now source shared message/update schemas from `@slopus/joyful-wire`.

### App (`packages/joyful-app`)

- Shared API message/update schemas in `sources/sync/apiTypes.ts` now import these from `@slopus/joyful-wire`:
  - `ApiMessageSchema`
  - `ApiUpdateNewMessageSchema`
  - `ApiUpdateSessionStateSchema`
  - `ApiUpdateMachineStateSchema`

### Server (`packages/joyful-server`)

- Prisma JSON message content type now references `SessionMessageContent` from `@slopus/joyful-wire`.
- Event router uses shared `SessionMessageContent` type for `new-message` payload typing.

### Agent (`packages/joyful-agent`)

- `RawMessage` now aliases `SessionMessage` from `@slopus/joyful-wire`.

## Versioning model

All other workspace packages now declare a versioned dependency on `@slopus/joyful-wire`.

This intentionally mirrors post-publish consumption and reduces hidden coupling to workspace-local files.

## Build and release

`@slopus/joyful-wire` is configured the same way as existing publishable libraries in this repo:
...
```bash
yarn workspace @slopus/joyful-wire release
```

When building workspaces from a clean checkout, build `@slopus/joyful-wire` first so dependent packages can resolve generated `dist` outputs.
...
3. Bump and release `@slopus/joyful-wire`.
4. Update downstream package versions if needed.
5. Publish dependent package updates only after the new `joyful-wire` version is available.

## Notes

- `joyful-wire` should stay focused on wire contracts only (types + Zod schemas + small helpers).
