...
## Overview
Integrate `@anthropic-ai/sandbox-runtime` into joyful-cli to sandbox both **Claude Code** and **Codex** sessions with OS-level filesystem and network restrictions. The sandbox wraps agent subprocesses, enforcing configurable restrictions without requiring containers.

Key features:
- **`joyful sandbox configure`** - Interactive CLI wizard (using `inquirer`) to set up sandbox rules
- **`joyful sandbox status`** - Show current sandbox configuration
- **`joyful sandbox disable`** - Turn off sandboxing
- **Automatic enforcement** - Once configured, sandbox wraps both Claude and Codex sessions by default (bypass with `--no-sandbox`)
- **Global config** - Stored in `~/.joyful/settings.json` alongside existing settings
- **Dual agent support** - Same sandbox config applies to both Claude Code and Codex

## Context
- **Claude spawn point**: `packages/joyful-cli/src/claude/claudeLocal.ts:241` - `spawn('node', [claudeCliPath, ...args], {env, ...})`
- **Codex spawn point**: `packages/joyful-cli/src/codex/codexMcpClient.ts:107` - `StdioClientTransport({ command: 'codex', args: ['mcp-server'], env })` which internally calls `cross-spawn('codex', ['mcp-server'])`
- **Config storage**: `packages/joyful-cli/src/persistence.ts` - Settings interface + Zod schemas + atomic `updateSettings()`
- **Command dispatch**: `packages/joyful-cli/src/index.ts` - manual `if/else if` routing on `args[0]`
- **Existing command pattern**: `packages/joyful-cli/src/commands/connect.ts` - exported `handleXxxCommand(args)` functions
...
- [x] Run `yarn add @anthropic-ai/sandbox-runtime inquirer` in `packages/joyful-cli`
- [x] Run `yarn add -D @types/inquirer` in `packages/joyful-cli`
...
- [x] Create `packages/joyful-cli/src/sandbox/config.ts`
...
- [x] Create `packages/joyful-cli/src/sandbox/manager.ts`
...
### Task 5: Create `joyful sandbox configure` interactive wizard
- [x] Create `packages/joyful-cli/src/commands/sandbox.ts`
...
### Task 6: Implement `joyful sandbox status` and `joyful sandbox disable`
...
### Task 10: Add `joyful sandbox` to help text and polish
- [x] Add `joyful sandbox` to the help text in `index.ts` (alongside `auth`, `connect`, `daemon`, etc.)
...
- [x] Verify `joyful sandbox configure` walks through all questions and saves config (automated command tests)
- [x] Verify `joyful sandbox status` shows current config (automated command tests)
- [x] Verify `joyful sandbox disable` turns off sandbox (automated command tests)
...
- ⚠️ Lint blocker: `packages/joyful-cli` has no `eslint.config.*` / `.eslintrc*`, so ESLint 9 cannot run in this package.
...
**Manual verification:**
- Test `joyful sandbox configure` end-to-end on macOS
