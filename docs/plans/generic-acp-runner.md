...
- **Session protocol types** in `@slopus/joyful-wire` (`SessionEnvelope`, `createEnvelope()`).
...
**File**: `packages/joyful-cli/src/agent/acp/AcpSessionMapper.ts`
...
**Test file**: `packages/joyful-cli/src/agent/acp/AcpSessionMapper.test.ts`
...
**File**: `packages/joyful-cli/src/agent/acp/runAcp.ts`
...
2. Start Joyful MCP server for tool bridge
...
Wire the generic runner into the CLI so users can run `joyful acp gemini` or `joyful acp opencode` or `joyful acp -- custom-agent --flag`.

**Files**:
- `packages/joyful-cli/src/index.ts` — add CLI command routing
...
- [ ] Add CLI routing for `joyful acp <agent-name>` and `joyful acp -- <cmd> [args]`
