...
⚠️ No lint script is defined in `packages/joyful-server/package.json`, so linter execution is currently not available.
...
- Update `joyful-agent` to use `POST /v3/sessions/:id/messages` instead of Socket.IO `message` event
- Update `joyful-agent` to poll `GET /v3/sessions/:id/messages` for receiving messages
