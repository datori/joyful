...
Implement `docs/session-protocol.md` as the new message format for Codex sessions in the CLI, and add client-side support in `joyful-app` to parse, normalize, and render these messages alongside existing legacy formats (output, codex, acp).
...
- ⚠️ `packages/joyful-cli` and `packages/joyful-app` do not define a `lint` script; verification used full test suites plus `yarn typecheck` in both packages.
...
- [x] Create `packages/joyful-cli/src/sessionProtocol/types.ts` with:
...
- [x] Add `sendSessionProtocolMessage(envelope)` to `ApiSessionClient` in `packages/joyful-cli/src/api/apiSession.ts`
...
- [x] Run full test suite — `yarn test` in joyful-cli, joyful-app
