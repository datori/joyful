...
## Overview
Create a portable, self-contained distribution of joyful-server as a single Bun-compiled binary. It runs without Redis (already has in-memory event bus), uses PGlite for embedded PostgreSQL, and local filesystem for file storage. CLI provides `joyful-server migrate` and `joyful-server serve` commands.
...
- [ ] Add `@electric-sql/pglite`, `pglite-prisma-adapter` to joyful-server dependencies
...
- [ ] Add `build:standalone` script to `package.json`: `bun build ./sources/standalone.ts --compile --outfile joyful-server`
...
- [ ] Test: `./joyful-server migrate` creates and migrates a PGlite database
- [ ] Test: `./joyful-server serve` starts the server with PGlite

### Task 7: Verify end-to-end
- [ ] Build the binary
- [ ] Run `./joyful-server migrate` — verify database is created in `./data/`
- [ ] Run `./joyful-server serve` — verify server starts, health endpoint responds
...
**CLI usage:**
```
joyful-server migrate    # Apply database migrations
joyful-server serve      # Start the server
```
...
- Consider adding `joyful-server init` command for first-time setup
