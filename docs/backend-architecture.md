...
This document describes the Joyful backend structure as implemented in `packages/joyful-server`. It focuses on how the server is wired, how data flows through the system, and which subsystems handle which responsibilities.
...
## Process lifecycle
Entry point: `packages/joyful-server/sources/main.ts`.
...
- Entrypoint: `packages/joyful-server/sources/main.ts`
- API server: `packages/joyful-server/sources/app/api/api.ts`
- Socket server: `packages/joyful-server/sources/app/api/socket.ts`
- Event routing: `packages/joyful-server/sources/app/events/eventRouter.ts`
- Presence: `packages/joyful-server/sources/app/presence`
- Storage: `packages/joyful-server/sources/storage`
- Prisma schema: `packages/joyful-server/prisma/schema.prisma`
