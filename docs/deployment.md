...
This document describes how to deploy the Joyful backend (`packages/joyful-server`) and the infrastructure it expects.
...
Example manifests live in `packages/joyful-server/deploy`:
- `handy.yaml`: Deployment + Service + ExternalSecrets for the server.
- `joyful-redis.yaml`: Redis StatefulSet + Service + ConfigMap.
...
- `yarn workspace joyful-server db` (Postgres in Docker)
- `yarn workspace joyful-server redis`
- `yarn workspace joyful-server s3` + `s3:init`

Use `.env`/`.env.dev` to load local settings when running `yarn workspace joyful-server dev`.

## Implementation references
- Entrypoint: `packages/joyful-server/sources/main.ts`
- Dockerfile: `Dockerfile.server`
- Kubernetes manifests: `packages/joyful-server/deploy`
- Env usage: `packages/joyful-server/sources` (`rg -n "process.env"`)
