...
This document details how client data is encrypted, how encrypted blobs are structured, and how those blobs map onto protocol fields. It is based on `packages/joyful-cli/src/api/encryption.ts` and the server routes that accept/emit these values.
...
These are the client-side structures that get encrypted and sent over the wire. They are defined in `packages/joyful-cli/src/api/types.ts`.
...
- Client crypto: `packages/joyful-cli/src/api/encryption.ts`
- Session message format: `packages/joyful-cli/src/api/types.ts`
- Server message ingestion: `packages/joyful-server/sources/app/api/socket/sessionUpdateHandler.ts`
- Artifact/KV routes: `packages/joyful-server/sources/app/api/routes/artifactsRoutes.ts`, `packages/joyful-server/sources/app/kv/kvMutate.ts`
