...
This document describes the Joyful CLI (`packages/joyful-cli`) and its daemon. The CLI is both an interactive tool and a background session manager that keeps machine state in sync with the server.
...
- **Persistence/config:** `src/persistence.ts` + `src/configuration.ts` manage local state in `~/.joyful`.
...
```mermaid
flowchart TD
    Start([joyful ...]) --> Parse[Parse subcommand]
...
graph LR
    ...
    subgraph "Environment Overrides"
        direction TB
        E1[JOYFUL_HOME_DIR]
        E2[JOYFUL_SERVER_URL]
        E3[JOYFUL_WEBAPP_URL]
        E4[JOYFUL_VARIANT]
        E5[JOYFUL_EXPERIMENTAL]
        E6[JOYFUL_DISABLE_CAFFEINATE]
    end

    E1 -.-> settings & access & daemon & logs
```

Local state lives under `~/.joyful` (or `JOYFUL_HOME_DIR`):
...
- `JOYFUL_SERVER_URL` and `JOYFUL_WEBAPP_URL` override defaults.
- `JOYFUL_VARIANT`, `JOYFUL_EXPERIMENTAL`, `JOYFUL_DISABLE_CAFFEINATE` control behavior.
...
- CLI entry: `packages/joyful-cli/src/index.ts`
- Daemon: `packages/joyful-cli/src/daemon`
- Control server/client: `packages/joyful-cli/src/daemon/controlServer.ts`, `packages/joyful-cli/src/daemon/controlClient.ts`
- API clients: `packages/joyful-cli/src/api`
- Persistence: `packages/joyful-cli/src/persistence.ts`
- Config: `packages/joyful-cli/src/configuration.ts`
