/**
 * Chinese (Simplified) translations for the Joyful app
 * Values can be:
...
export const zhHans: TranslationStructure = {
    ...
    inbox: {
        // Inbox screen
        emptyTitle: '暂无更新',
        emptyDescription: '您已是最新状态，稍后再查看。',
        updates: '更新',
    },
    ...
    settings: {
        ...
        about: '关于',
        aboutFooter: 'Joyful Coder 是一个 Codex 和 Claude Code 移动客户端。它采用端到端加密，您的账户仅存储在本地设备上。与 Anthropic 无关联。',
        whatsNew: '更新日志',
    ...
    server: {
        ...
        notValidHappyServer: '不是有效的 Joyful 服务器',
        changeServer: '更改服务器',
    ...
    sessionInfo: {
        ...
        joyfulSessionIdCopied: 'Joyful 会话 ID 已复制到剪贴板',
        failedToCopySessionId: '复制 Joyful 会话 ID 失败',
        joyfulSessionId: 'Joyful 会话 ID',
        ...
        updateCliInstructions: '请运行 npm install -g joyful@latest',
        deleteSession: '删除会话',
    ...
    agentInput: {
        ...
        effort: {
            title: '推理强度',
            default: '默认',
            defaultDescription: '使用 Claude 默认设置',
            low: '低',
            medium: '中',
            high: '高',
            max: '最大',
            badge: ({ level }: { level: string }) => `强度: ${level}`,
        },
        codexPermissionMode: {
            title: 'CODEX 权限模式',
            default: 'CLI 设置',
    ...
    terminal: {
        ...
        terminalRequestDescription: '有终端正在请求连接到您的 Joyful Coder 账户。这将允许终端安全地发送和接收消息。',
    ...
    machine: {
        ...
        offlineHelp: '• 确保您的计算机在线\n• 运行 `joyful daemon status` 进行诊断\n• 您是否在运行最新的 CLI 版本？请使用 `npm install -g joyful@latest` 升级',
        daemon: '守护进程',
        status: '状态',
        stopDaemon: '停止守护进程',
