...
export const ja: TranslationStructure = {
    ...
    newSession: {
        ...
        worktree: {
            ...
            pickWorktree: 'ワークツリーを選択',
            createNew: '新しいワークツリーを作成',
            existingWorktrees: '既存のワークツリー',
    ...
    sessionInfo: {
        ...
        codexConversationIdCopied: 'Codex Conversation IDがクリップボードにコピーされました',
        ...
        failedToCopyCodexConversationId: 'Codex Conversation IDのコピーに失敗しました',
        ...
        worktreeCleanupFailed: ({ path }: { path: string }) => `ワークツリーのクリーンアップに失敗しました。手動で削除が必要な場合があります: ${path}`,
        worktreeSection: 'ワークツリー',
        worktreeBranch: 'ブランチ',
        worktreeBasePath: 'ベースリポジトリ',
        mergeToMain: 'メインにマージ',
        mergeToMainSubtitle: 'このブランチをベースリポジトリに統合する',
    ...
    mergeWorktree: {
        title: 'メインにマージ',
        preview: 'マージする変更',
        commits: ({ count }: { count: number }) => `${count} コミット`,
        noChanges: 'マージする変更なし',
        loadingDiff: 'diff を読み込み中...',
        viewFullDiff: '完全な diff を表示',
        fullDiffTitle: '完全な diff',
        squashMerge: 'スカッシュマージ（推奨）',
        regularMerge: '通常のマージ',
        squashDescription: 'すべてのコミットをベースブランチの1つのクリーンなコミットにまとめます',
        regularDescription: 'ワークツリーブランチのコミット履歴をすべて保持します',
        commitMessageLabel: 'コミットメッセージ',
        commitMessageDefault: ({ branchName }: { branchName: string }) => `エージェントタスク: ${branchName}`,
        mergeButton: 'マージ',
        merging: 'マージ中...',
        successTitle: 'マージ成功',
        conflictTitle: 'マージの競合',
        conflictMessage: 'マージで競合が発生し、中断されました。ターミナルで手動で解決してください。',
        conflictFilesLabel: '競合しているファイル',
        worktreeDirtyMessage: 'ワークツリーにコミットされていない変更があります。マージする前にコミットまたは破棄してください。',
        baseDirtyMessage: 'ベースリポジトリにコミットされていない変更があります。マージする前にコミットまたは破棄してください。',
        cleanupTitle: 'ワークツリーをクリーンアップしますか？',
        cleanupMessage: 'マージが成功しました。ワークツリーのディレクトリとブランチを削除しますか？',
        cleanupConfirm: 'ワークツリーとブランチを削除',
        openspecUnarchivedTitle: 'アーカイブされていない OpenSpec の変更',
        openspecUnarchivedMessage: '以下の OpenSpec の変更はまだアーカイブされていません。仕様ファイルが最新であることを確認するために、マージ前にアーカイブすることを検討してください:',
        openspecArchiveFirst: '最初にアーカイブ（推奨）',
        openspecProceedAnyway: 'このまま続行',
        specDivergenceTitle: '仕様ファイルが分岐しています',
        specDivergenceMessage: 'このワークツリーが分岐した後、main の openspec/specs/ ディレクトリが更新されました。今マージすると仕様ファイルで競合が発生する可能性があります。',
        specPullResync: 'main を取得して再同期',
        specPullResyncDescription: 'このブランチに main を取り込み、/opsx:sync を実行して仕様の競合を解決するよう促します。',
        specMergeAnyway: 'それでもマージ',
        specAfterPullMessage: 'main を取得した後、このセッションで /opsx:sync を実行して仕様の競合を解決し、マージを再試行してください。',
        pullMainSuccess: 'メインブランチの取得に成功しました。/opsx:sync を実行して仕様の競合を解決してください。',
        pullMainFailed: 'メインブランチの取得に失敗しました',
        orphanTitle: '孤立したワークツリー',
        orphanNoSession: 'アクティブなセッションなし',
        orphanCleanup: 'クリーンアップ',
        orphanCleanupAll: 'すべてクリーンアップ',
        orphanDirtyTitle: 'コミットされていない変更',
        ...
        mergeWithAI: 'AIでマージ',
        mergeWithAISubtitle: 'エージェントがスペックを確認し、競合を解決してmainにマージします',
        returnToMerge: 'マージ画面に戻る',
        resolveWithAI: 'AIで解決',
        resolveWithAISubtitle: 'コンフリクト解決プロンプトでセッションを再開',
        syncSpecsWithAI: 'AIでスペックを同期',
        syncSpecsWithAISubtitle: '現在のスペックに合わせて実装を更新するためにセッションを再開',
        fetchingSpecDiff: 'スペックのdiffを取得中...',
    },
} as const;
