...
```bash
git clone https://github.com/slopus/joyful-cli.git
cd joyful-cli
yarn install
yarn build
```
...
### Global `joyful-dev` Command

Create a global `joyful-dev` command that runs your local development build:

```bash
yarn link:dev      # Create joyful-dev symlink
yarn unlink:dev    # Remove joyful-dev symlink
```

This creates a `joyful-dev` command in your PATH pointing to your local build, while leaving any npm-installed `joyful` command untouched.

| Command | Runs |
|---------|------|
| `joyful` | Stable npm version (from `npm install -g joyful`) |
| `joyful-dev` | Local development version (from this repo) |
...
This creates:
- `~/.joyful/` - Stable version data (production-ready)
- `~/.joyful-dev/` - Development version data (for testing changes)
...
- `✅ STABLE MODE - Data: ~/.joyful`
- `🔧 DEV MODE - Data: ~/.joyful-dev`
...
|--------|--------|-------------|
| Data Directory | `~/.joyful/` | `~/.joyful-dev/` |
| Settings | `~/.joyful/settings.json` | `~/.joyful-dev/settings.json` |
| Auth Keys | `~/.joyful/access.key` | `~/.joyful-dev/access.key` |
| Daemon State | `~/.joyful/daemon.state.json` | `~/.joyful-dev/daemon.state.json` |
| Logs | `~/.joyful/logs/` | `~/.joyful-dev/logs/` |
...
3. Now `cd` into the directory automatically sets `JOYFUL_VARIANT=dev`!
...
```bash
npm run stable:daemon:status   # Shows ~/.joyful/ data location
npm run dev:daemon:status       # Shows ~/.joyful-dev/ data location
```
...
### `joyful-dev` command not found after linking?
- Ensure your global npm bin is in PATH: `npm bin -g`
- Try opening a new terminal window
- Check the symlink was created: `ls -la $(npm bin -g)/joyful-dev`
...
4. **Different accounts** - Use different Joyful accounts for dev/stable if needed
5. **Check logs** - Logs are separated: `~/.joyful/logs/` vs `~/.joyful-dev/logs/`
...
The system uses the built-in `JOYFUL_HOME_DIR` environment variable to separate data:

- **Stable scripts** set: `JOYFUL_HOME_DIR=~/.joyful`
- **Dev scripts** set: `JOYFUL_HOME_DIR=~/.joyful-dev`

Everything else (auth, sessions, logs, daemon) automatically follows the `JOYFUL_HOME_DIR` setting.

Cross-platform via Node.js - works identically on Windows, macOS, and Linux!

## Testing Profile Sync Between GUI and CLI

Profile synchronization ensures AI backend configurations created in the Joyful mobile/web GUI work seamlessly with the CLI daemon.
...
2. **Verify CLI receives profile:**
   ...
   # Check daemon logs
   tail -f ~/.joyful-dev/logs/*.log | grep -i profile
   ```
...
# or directly:
yarn workspace joyful release
```

This runs tests, builds, and publishes to npm. The published package includes:
- `joyful` - Main CLI command
- `joyful-mcp` - MCP bridge command

**Note:** `joyful-dev` is intentionally excluded from the npm package — it's for local development only.
