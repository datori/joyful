#!/usr/bin/env node
/**
 * link-dev.cjs - Create symlink for joyful-dev only
 *
 * This script creates a symlink for the joyful-dev command pointing to the local
 * development version, while leaving the stable npm version of `joyful` untouched.
 *
 * Usage: yarn link:dev
 *
 * What it does:
 * 1. Finds the global npm bin directory
 * 2. Creates/updates a symlink: joyful-dev -> ./bin/joyful-dev.mjs
 *
...
const binSource = join(projectRoot, 'bin', 'joyful-dev.mjs');
...
function link() {
    ...
    const binTarget = join(globalBin, 'joyful-dev');

    console.log('Creating symlink for joyful-dev...');
    ...
    try {
        ...
        console.log('\n✅ Successfully linked joyful-dev to local development version');
        console.log('\nNow you can use:');
        console.log('  joyful      → stable npm version (unchanged)');
        console.log('  joyful-dev  → local development version');
        console.log('\nTo undo: yarn unlink:dev');
    } catch (e) {
        if (e.code === 'EACCES') {
...
function unlink() {
    ...
    console.log('Removing joyful-dev symlink...');

    try {
        const stat = fs.lstatSync(binTarget);
        if (stat.isSymbolicLink()) {
            const linkTarget = fs.readlinkSync(binTarget);
            if (linkTarget === binSource || linkTarget.includes('joyful-cli')) {
                fs.unlinkSync(binTarget);
                console.log('\n✅ Removed joyful-dev development symlink');
                console.log('\nTo restore npm version: npm install -g joyful');
            } else {
                console.log(`\n⚠️  joyful-dev symlink points elsewhere: ${linkTarget}`);
        ...
        if (e.code === 'ENOENT') {
            console.log("\n✅ joyful-dev symlink doesn't exist (already removed or never created)");
