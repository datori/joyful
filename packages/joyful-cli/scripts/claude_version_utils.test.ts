...
describe('JOYFUL_CLAUDE_PATH env var', () => {
  ...
  afterEach(() => {
    ...
    delete process.env.JOYFUL_CLAUDE_PATH;
  });

  it('should use JOYFUL_CLAUDE_PATH when set', () => {
    process.env.JOYFUL_CLAUDE_PATH = testClaudePath;
    const result = findGlobalClaudeCliPath();
    expect(result?.source).toBe('JOYFUL_CLAUDE_PATH');
  ...
  it('should fall back to auto-discovery when env var not set', () => {
    const result = findGlobalClaudeCliPath();
    expect(result?.source).not.toBe('JOYFUL_CLAUDE_PATH');
  });

  it('should ignore env var if path does not exist', () => {
    process.env.JOYFUL_CLAUDE_PATH = '/nonexistent/path/claude';
    const result = findGlobalClaudeCliPath();
    expect(result?.source).not.toBe('JOYFUL_CLAUDE_PATH');
  });
});