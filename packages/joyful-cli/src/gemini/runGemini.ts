...
export async function runGemini(opts: {
  ...
  //
  // Fetch Gemini cloud token (from 'joyful connect gemini')
  //
  ...
  try {
    ...
    if (vendorToken?.oauth?.access_token) {
      ...
      logger.debug('[Gemini] Using OAuth token from Joyful server');
    ...
    switch (msg.type) {
        ...
        if (msg.status === 'running') {
          ...
          if (errorMessage.includes('Authentication required')) {
            errorMessage = `Authentication required.\n` +
              `For Google Workspace accounts, run: joyful gemini project set <project-id>\n` +
              `Or use a different Google account: joyful connect gemini\n` +
      ...
      case 'tool-result':
        ...
        if (msg.toolName === 'change_title' || 
            ...
            msg.toolName === 'joyful__change_title') {
    ...
    while (!shouldExit) {
      ...
      try {
        ...
        pendingChangeTitle = message.message.includes('change_title') || 
                             message.message.includes('joyful__change_title');
