...
export async function handleSandboxStatus(): Promise<void> {
    ...
    if (!config) {
        console.log('Sandbox is not configured. Run `joyful sandbox configure`.');
        return;
    }
...
export function handleSandboxHelp(): void {
    console.log(`
${chalk.bold('joyful sandbox')} - Sandbox management

${chalk.bold('Usage:')}
  joyful sandbox configure      Configure sandbox settings interactively
  joyful sandbox status         Show current sandbox configuration
  joyful sandbox disable        Disable sandboxing
  joyful sandbox help           Show this help
`);
}
