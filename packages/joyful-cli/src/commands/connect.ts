 ...
 * - connect codex: Store OpenAI API key in Joyful server
 * - connect claude: Store Anthropic API key in Joyful server
 * - connect gemini: Store Gemini API key in Joyful server
...
function showConnectHelp(): void {
    console.log(`
${chalk.bold('joyful connect')} - Connect AI vendor API keys to Joyful server

${chalk.bold('Usage:')}
  joyful connect codex        Store your Codex API key in Joyful server
  joyful connect claude       Store your Anthropic API key in Joyful server
  joyful connect gemini       Store your Gemini API key in Joyful server
  joyful connect status       Show connection status for all vendors
  joyful connect help         Show this help message

${chalk.bold('Description:')}
  The connect command allows you to securely store your AI vendor API keys
  in Joyful server. This enables you to use these services through Happy
  without exposing your API keys locally.

${chalk.bold('Examples:')}
  joyful connect codex
  joyful connect claude
  joyful connect gemini
  joyful connect status

${chalk.bold('Notes:')} 
  • You must be authenticated with Happy first (run 'joyful auth login')
  • API keys are encrypted and stored securely in Joyful server
  • You can manage your stored keys at your-joyful-server
`);
}

async function handleConnectVendor(vendor: 'codex' | 'claude' | 'gemini', displayName: string): Promise<void> {
    console.log(chalk.bold(`\n🔌 Connecting ${displayName} to Joyful server\n`));

    // Check if authenticated
    const credentials = await readCredentials();
    if (!credentials) {
        console.log(chalk.yellow('⚠️  Not authenticated with Happy'));
        console.log(chalk.gray('  Please run "joyful auth login" first'));
        process.exit(1);
    }
...
async function handleConnectStatus(): Promise<void> {
    ...
    console.log('');
    console.log(chalk.gray('To connect a vendor, run: joyful connect <vendor>'));
    console.log(chalk.gray('Example: joyful connect gemini'));
    console.log('');
}

/**
 * Update local Gemini credentials file to keep in sync with Joyful server
