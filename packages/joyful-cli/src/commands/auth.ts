...
function showAuthHelp(): void {
  console.log(`
${chalk.bold('joyful auth')} - Authentication management

${chalk.bold('Usage:')}
  joyful auth login [--force]    Authenticate with Joyful
  joyful auth logout             Remove authentication and machine data
  joyful auth status             Show authentication status
  joyful auth help               Show this help message
...
async function handleAuthLogin(args: string[]): Promise<void> {
  ...
  if (!forceAuth) {
    ...
    if (existingCreds && settings?.machineId) {
      ...
      console.log(chalk.gray(`  Use 'joyful auth login --force' to re-authenticate`));
      return;
...
async function handleAuthLogout(): Promise<void> {
  ...
  const joyfulDir = configuration.joyfulHomeDir;
  ...
  if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
    try {
      ...
      // Remove entire joyful directory (as current logout does)
      if (existsSync(joyfulDir)) {
        rmSync(joyfulDir, { recursive: true, force: true });
      }

      console.log(chalk.green('✓ Successfully logged out'));
      console.log(chalk.gray('  Run "joyful auth login" to authenticate again'));
    } catch (error) {
...
async function handleAuthStatus(): Promise<void> {
  ...
  if (!credentials) {
    ...
    console.log(chalk.gray('  Run "joyful auth login" to authenticate'));
    return;
  }
  ...
  if (settings?.machineId) {
    ...
    console.log(chalk.gray('  Run "joyful auth login --force" to fix this'));
  }

  // Data location
  console.log(chalk.gray(`\n  Data directory: ${configuration.joyfulHomeDir}`));
