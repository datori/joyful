 ...
 *
 * Configure the target HTTP MCP URL via env var `JOYFUL_HTTP_MCP_URL` or
...
async function main() {
  ...
  const baseUrl = urlFromArgs || process.env.JOYFUL_HTTP_MCP_URL || '';

  if (!baseUrl) {
    // Write to stderr; never stdout.
    process.stderr.write(
      '[joyful-mcp] Missing target URL. Set JOYFUL_HTTP_MCP_URL or pass --url <http://127.0.0.1:PORT>\n'
    );
    process.exit(2);
  }
  ...
  async function ensureHttpClient(): Promise<Client> {
    ...
    const client = new Client(
      { name: 'joyful-stdio-bridge', version: '1.0.0' },
      { capabilities: {} }
    );
...
main().catch((err) => {
  try {
    process.stderr.write(`[joyful-mcp] Fatal: ${err instanceof Error ? err.message : String(err)}\n`);
  } finally {
    process.exit(1);
  }
});
