...
export class ApiClient {
  ...
  private constructor(credential: Credentials) {
    ...
    this.pushClient = new PushNotificationClient(credential.token, configuration.requireServerUrl())
  }
  ...
  async getOrCreateSession(opts: {
    ...
    try {
      const response = await axios.post<CreateSessionResponse>(
        `${configuration.requireServerUrl()}/v1/sessions`,
        {
          tag: opts.tag,
      ...
      if (error && typeof error === 'object' && 'code' in error) {
        ...
        if (isNetworkError(errorCode)) {
          connectionState.fail({
            ...
            errorCode,
            url: `${configuration.requireServerUrl()}/v1/sessions`
          });
          return null;
        }
      }
  ...
  async getOrCreateMachine(opts: {
    ...
    try {
      const response = await axios.post(
        `${configuration.requireServerUrl()}/v1/machines`,
        {
          id: opts.machineId,
          metadata: encodeBase64(encrypt(encryptionKey, encryptionVariant, opts.metadata)),
      ...
      if (axios.isAxiosError(error) && error.code && isNetworkError(error.code)) {
        connectionState.fail({
          ...
          url: `${configuration.requireServerUrl()}/v1/machines`
        });
      ...
      if (axios.isAxiosError(error) && error.response?.status) {
        ...
        if (status === 403 || status === 409) {
          ...
          console.log(chalk.yellow(
            `   → Run  to reset local state and generate a new machine ID`
          ));
          console.log(chalk.yellow(
  ...
  async registerVendorToken(vendor: 'openai' | 'anthropic' | 'gemini', apiKey: any): Promise<void> {
    try {
      const response = await axios.post(
        `${configuration.requireServerUrl()}/v1/connect/${vendor}/register`,
        {
  ...
  async getVendorToken(vendor: 'openai' | 'anthropic' | 'gemini'): Promise<any | null> {
    try {
      const response = await axios.get(
        `${configuration.requireServerUrl()}/v1/connect/${vendor}/token`,
        {
          headers: {
            'Authorization': `Bearer ${this.credential.token}`,
            'Content-Type': 'application/json'
