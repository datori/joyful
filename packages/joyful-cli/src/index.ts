...
/**
 * CLI entry point for joyful command
 * 
...
(async () => {
  ...
  if (!args.includes('--version')) {
    logger.debug('Starting joyful CLI with args: ', process.argv)
  }
  ...
  if (subcommand === 'doctor') {
    ...
    // Handle "joyful gemini model set <model>" command
    ...
    // Handle "joyful gemini model get" command
    ...
    // Handle "joyful gemini project set <project-id>" command
    ...
    // Handle "joyful gemini project get" command
    if (geminiSubcommand === 'project' && args[2] === 'get') {
      try {
        ...
        if (config.googleCloudProject) {
          ...
          console.log('  joyful gemini project set <your-project-id>');
          console.log('');
    ...
    // Handle "joyful gemini project" (no subcommand) - show help
    if (geminiSubcommand === 'project' && !args[2]) {
      console.log('Usage: joyful gemini project <command>');
      console.log('');
    ...
    console.log(chalk.yellow('Note: "joyful logout" is deprecated. Use "joyful auth logout" instead.\n'));
    try {
    ...
    if (daemonSubcommand === 'list') {
      ...
      console.log(`
${chalk.bold('joyful daemon')} - Daemon management

${chalk.bold('Usage:')}
  joyful daemon start              Start the daemon (detached)
  joyful daemon stop               Stop the daemon (sessions stay alive)
  joyful daemon status             Show daemon status
  joyful daemon list               List active sessions

  If you want to kill all joyful related processes run 
  ${chalk.cyan('joyful doctor clean')}

${chalk.bold('Note:')} The daemon runs in the background and manages Claude sessions.

${chalk.bold('To clean up runaway processes:')} Use ${chalk.cyan('joyful doctor clean')}
`)
    }
    return;
  } else {
    ...
    for (let i = 0; i < args.length; i++) {
      ...
      if (arg === '-h' || arg === '--help') {
        ...
        unknownArgs.push(arg)
      } else if (arg === '--joyful-starting-mode') {
    ...
    if (showHelp) {
      console.log(`
${chalk.bold('joyful')} - Claude Code On the Go

${chalk.bold('Usage:')}
  joyful [options]         Start Claude with mobile control
  joyful auth              Manage authentication
  joyful codex             Start Codex mode
  joyful gemini            Start Gemini mode (ACP)
  joyful acp               Start a generic ACP-compatible agent
  joyful connect           Connect AI vendor API keys
  joyful sandbox           Configure and manage OS-level sandboxing
  joyful notify            Send push notification
  joyful daemon            Manage background service that allows
                            to spawn new sessions away from your computer
  joyful doctor            System diagnostics & troubleshooting

${chalk.bold('Examples:')}
  joyful                   Start session
  joyful --yolo             Start with bypassing permissions
                            joyful sugar for --dangerously-skip-permissions
  joyful --chrome           Enable Chrome browser access for this session
  joyful --no-chrome        Disable Chrome even if default is on
  joyful --no-sandbox       Disable Joyful sandbox for this session
  joyful --js-runtime bun   Use bun instead of node to spawn Claude Code
  joyful --claude-env ANTHROPIC_BASE_URL=http://127.0.0.1:3456
                           Use a custom API endpoint (e.g., claude-code-router)
  joyful acp gemini         Start Gemini via generic ACP runner
  joyful acp -- opencode --acp
                           Start a custom ACP command
  joyful acp opencode --verbose
                           Print raw ACP backend/envelope events
  joyful auth login --force Authenticate
  joyful doctor             Run diagnostics

${chalk.bold('Joyful supports ALL Claude options!')}
  Use any claude flag with joyful as you would with claude. Our favorite:
...
`)
    ...
    // Show version
    if (showVersion) {
      console.log(`joyful version: ${packageJson.version}`)
...
async function handleNotifyCommand(args: string[]): Promise<void> {
  ...
  if (showHelp) {
    console.log(`
${chalk.bold('joyful notify')} - Send notification

${chalk.bold('Usage:')}
  joyful notify -p <message> [-t <title>]    Send notification with custom message and optional title
  joyful notify -h, --help                   Show this help
...
${chalk.bold('Examples:')}
  joyful notify -p "Deployment complete!"
  joyful notify -p "System update complete" -t "Server Status"
  joyful notify -t "Alert" -p "Database connection restored"
`)
    return
  }
  ...
  if (!message) {
    ...
    console.log(chalk.gray('Run "joyful notify --help" for usage information.'))
    process.exit(1)
  }
  ...
  if (!credentials) {
    console.error(chalk.red('Error: Not authenticated. Please run "joyful auth login" first.'))
    process.exit(1)
  }
  ...
  try {
    ...
    const notificationTitle = title || 'Joyful'
