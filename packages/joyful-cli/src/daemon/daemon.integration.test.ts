 ...
 * - JOYFUL_HOME_DIR=~/.joyful-dev-test (DIFFERENT from dev's ~/.joyful-dev!)
 * - JOYFUL_SERVER_URL=http://localhost:3005 (local dev server)
 */
...
async function isServerHealthy(): Promise<boolean> {
  try {
    ...
    const testCredentials = existsSync(join(configuration.joyfulHomeDir, 'access.key'));
    if (!testCredentials) {
      console.log('[TEST] No test credentials found in', configuration.joyfulHomeDir);
      console.log('[TEST] Run "joyful auth login" with JOYFUL_HOME_DIR=~/.joyful-dev-test first');
      return false;
    }
...
describe.skipIf(!await isServerHealthy())('Daemon Integration Tests', { timeout: 20_000 }, () => {
  ...
  it('should track session-started webhook from terminal session', async () => {
    ...
    const mockMetadata: Metadata = {
      ...
      host: 'test-host',
      homeDir: '/test/home',
      joyfulHomeDir: '/test/joyful-home',
      joyfulLibDir: '/test/joyful-lib',
      joyfulToolsDir: '/test/joyful-tools',
      hostPid: 99999,
      startedBy: 'terminal',
    ...
    expect(tracked.startedBy).toBe('joyful directly - likely by user from terminal');
    expect(tracked.joyfulSessionId).toBe('test-session-123');
  ...
  it('should spawn & stop a session via HTTP (not testing RPC route, but similar enough)', async () => {
    ...
    const spawnedSession = sessions.find(
      (s: any) => s.joyfulSessionId === response.sessionId
    );
    ...
    expect(spawnedSession.joyfulSessionId).toBeDefined();
    await stopDaemonSession(spawnedSession.joyfulSessionId);
  });
  ...
  it('should track both daemon-spawned and terminal sessions', async () => {
    ...
    if (!terminalHappyProcess || !terminalHappyProcess.pid) {
      throw new Error('Failed to spawn terminal joyful process');
    }
    ...
    const daemonSession = sessions.find(
      (s: any) => s.joyfulSessionId === spawnResponse.sessionId
    );

    expect(terminalSession).toBeDefined();
    expect(terminalSession.startedBy).toBe('joyful directly - likely by user from terminal');
    ...
    await stopDaemonSession(daemonSession.joyfulSessionId);
  ...
  it('should update session metadata when webhook is called', async () => {
    ...
    const sessions = await listDaemonSessions();
    const session = sessions.find((s: any) => s.joyfulSessionId === spawnResponse.sessionId);
  ...
  it('should handle concurrent session operations', async () => {
    ...
    const daemonSessions = sessions.filter(
      (s: any) => s.startedBy === 'daemon' && spawnedSessionIds.includes(s.joyfulSessionId)
    );
    expect(daemonSessions.length).toBeGreaterThanOrEqual(3);

    // Stop all spawned sessions
    for (const session of daemonSessions) {
      expect(session.joyfulSessionId).toBeDefined();
      await stopDaemonSession(session.joyfulSessionId);
    }
  });
   ...
   * 
   * This simulates what happens during `npm upgrade joyful`:
   * - Running daemon has OLD version loaded in memory (configuration.currentCliVersion)
   * - npm replaces node_modules/joyful/ with NEW version files
  ...
  it('[takes 1 minute to run] should detect version mismatch and kill old daemon', { timeout: 100_000 }, async () => {
    ...
    try {
      ...
      // otherwise daemon will spawn a non existing joyful js script.
      ...
      await new Promise(resolve => setTimeout(resolve, parseInt(process.env.JOYFUL_DAEMON_HEARTBEAT_INTERVAL || '30000') + 10_000));
  ...
  // TODO: Test npm uninstall scenario - daemon should gracefully handle when joyful is uninstalled
