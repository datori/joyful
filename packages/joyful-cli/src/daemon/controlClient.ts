...
async function daemonPost(path: string, body?: any): Promise<{ error?: string } | any> {
  ...
  try {
    const timeout = process.env.JOYFUL_DAEMON_HTTP_TIMEOUT ? parseInt(process.env.JOYFUL_DAEMON_HTTP_TIMEOUT) : 10_000;
 ...
 * For instance we are not handling the case where we upgraded joyful,
 ...
 * For instance when running `joyful daemon status` we can show more information.
 */
...
export async function isDaemonRunningCurrentlyInstalledHappyVersion(): Promise<boolean> {
  ...
  try {
    ...
    // we will get a new path or not when joyful is upgraded globally.
    ...
    await new Promise(resolve => joyfulProcess.stdout?.on('close', resolve));
