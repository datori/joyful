 ...
 * the daemon when the user runs the joyful command. 
 * 
 ...
 * 2. We assume users will run joyful frequently (every time they open their laptop)
...
const PLIST_LABEL = 'com.joyful-cli.daemon';
...
export async function install(): Promise<void> {
    try {
        ...
        // Get the path to the joyful CLI executable
        const joyfulPath = process.argv[0]; // Node.js executable
        ...
        const plistContent = trimIdent(`
            ...
            <dict>
                ...
                <array>
                    <string>${joyfulPath}</string>
                    <string>${scriptPath}</string>
                    <string>joyful-daemon</string>
                </array>
                
                <key>EnvironmentVariables</key>
                <dict>
                    <key>JOYFUL_DAEMON_MODE</key>
                    <string>true</string>
                </dict>
                ...
                <string>${os.homedir()}/.joyful/daemon.err</string>
                
                <key>StandardOutPath</key>
                <string>${os.homedir()}/.joyful/daemon.log</string>
        ...
        logger.info('Check logs at ~/.joyful/daemon.log');
