# Joyful CLI Daemon: Control Flow and Lifecycle

The daemon is a persistent background process that manages Joyful sessions, enables remote control from the mobile app, and handles auto-updates when the CLI version changes.

## 1. Daemon Lifecycle

### Starting the Daemon

Command: `joyful daemon start`

Control Flow:
1. `src/index.ts` receives `daemon start` command
2. Spawns detached process via `spawnJoyfulCLI(['daemon', 'start-sync'], { detached: true })`
...
4. `startDaemon()` performs startup:
   ...
   - RPC registration: exposes `spawn-joyful-session`, `stop-session`, `requestShutdown` handlers
   - Heartbeat loop: every 60s (or JOYFUL_DAEMON_HEARTBEAT_INTERVAL) checks for version updates and prunes dead sessions
...
The daemon detects when `npm upgrade joyful` occurs:
...
3. If mismatch detected:
   - Spawns new daemon via `spawnJoyfulCLI(['daemon', 'start'])`
...
1. Backend forwards RPC `spawn-joyful-session` to daemon via WebSocket
2. `ApiMachineClient` invokes `spawnSession()` handler
3. `spawnSession()`:
   - Creates directory if needed
   - Spawns detached Joyful process with `--joyful-starting-mode remote --started-by daemon`
   - Adds to `pidToTrackedSession` map
   - Sets up 10-second awaiter for session webhook
4. New Joyful process:
   - Creates session with backend, receives `joyfulSessionId`
   - Calls `notifyDaemonSessionStarted()` to POST to daemon's `/session-started`
5. Daemon updates tracking with `joyfulSessionId`, resolves awaiter
6. RPC returns session info to mobile app

### Terminal-Spawned Sessions

User runs `joyful` directly:
1. CLI auto-starts daemon if configured
2. Joyful process calls `notifyDaemonSessionStarted()` 
3. Daemon receives webhook, creates `TrackedSession` with `startedBy: 'joyful directly...'`
4. Session tracked for health monitoring

### Session Termination

Via RPC `stop-session` or health check:
1. `stopSession()` finds session by `joyfulSessionId`
...
`joyful doctor` uses `ps aux | grep` to find all Joyful processes:
- Production: matches `joyful.mjs`, `joyful`, `dist/index.mjs`
...
`joyful doctor clean`:
1. `findRunawayJoyfulProcesses()` filters for likely orphans
2. `killRunawayJoyfulProcesses()`:
   - Sends SIGTERM
   - Waits 1 second
...
- Server to Daemon: rpc-request (spawn-joyful-session, stop-session, requestShutdown)
...
interface MachineMetadata {
  ...
  joyfulCliVersion: string;   
  homeDir: string;           
  joyfulHomeDir: string;
}
...
```json
{
  ...
  "metadata": "base64(encrypted({
    ...
    'homeDir': '/Users/john',
    'joyfulHomeDir': '/Users/john/.joyful'
  }))",
...
socket.emit('machine-update-metadata', {
  ...
  "metadata": "base64(encrypted({
    'host': 'MacBook-Pro.local',
    'platform': 'darwin',
    'joyfulCliVersion': '1.0.1',  // version updated
    'homeDir': '/Users/john',
    'joyfulHomeDir': '/Users/john/.joyful'
  }))",
  "expectedVersion": 1
