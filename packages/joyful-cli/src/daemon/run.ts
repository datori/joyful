...
export async function startDaemon(): Promise<void> {
  ...
  let requestShutdown: (source: 'joyful-app' | 'joyful-cli' | 'os-signal' | 'exception', errorMessage?: string) => void;
  let resolvesWhenShutdownRequested = new Promise<({ source: 'joyful-app' | 'joyful-cli' | 'os-signal' | 'exception', errorMessage?: string })>((resolve) => {
  ...
  try {
    ...
    // Handle webhook from joyful session reporting itself
    const onHappySessionWebhook = (sessionId: string, sessionMetadata: Metadata) => {
      ...
      if (existingSession && existingSession.startedBy === 'daemon') {
        ...
        existingSession.joyfulSessionId = sessionId;
        existingSession.joyfulSessionMetadataFromLocalWebhook = sessionMetadata;
        ...
        const trackedSession: TrackedSession = {
          startedBy: 'joyful directly - likely by user from terminal',
          joyfulSessionId: sessionId,
          joyfulSessionMetadataFromLocalWebhook: sessionMetadata,
          pid
        };
    ...
    const spawnSession = async (options: SpawnSessionOptions): Promise<SpawnSessionResult> => {
      ...
      try {
        ...
        if (useTmux && tmuxSessionName !== undefined) {
          ...
          const fullCommand = `node --no-warnings --no-deprecation ${cliPath} ${agent} --joyful-starting-mode remote --started-by daemon`;
          ...
          const windowName = `joyful-${Date.now()}-${agent}`;
          ...
          if (tmuxResult.success) {
            ...
            // Wait for webhook to populate session with joyfulSessionId (exact same as regular flow)
            ...
            return new Promise((resolve) => {
              ...
              pidToAwaiter.set(tmuxResult.pid!, (completedSession) => {
                ...
                logger.debug(`[DAEMON RUN] Session ${completedSession.joyfulSessionId} fully spawned with webhook (tmux)`);
                resolve({
                  type: 'success',
                  sessionId: completedSession.joyfulSessionId!
                });
              });
            });
          } else {
        ...
        if (!useTmux) {
          ...
          if (process.env.DEBUG) {
            ...
            joyfulProcess.stderr?.on('data', (data) => {
              logger.debug(`[DAEMON RUN] Child stderr: ${data.toString()}`);
            });
          }

          if (!joyfulProcess.pid) {
          ...
          logger.debug(`[DAEMON RUN] Spawned process with PID ${joyfulProcess.pid}`);

          const trackedSession: TrackedSession = {
            startedBy: 'daemon',
            pid: joyfulProcess.pid,
            childProcess: joyfulProcess,
            directoryCreated,
            message: directoryCreated ? `The path '${directory}' did not exist. We created a new folder and spawned a new session there.` : undefined
          };

          pidToTrackedSession.set(joyfulProcess.pid, trackedSession);

          joyfulProcess.on('exit', (code, signal) => {
            logger.debug(`[DAEMON RUN] Child PID ${joyfulProcess.pid} exited with code ${code}, signal ${signal}`);
            if (joyfulProcess.pid) {
              onChildExited(joyfulProcess.pid);
            }
          });

          joyfulProcess.on('error', (error) => {
          ...
          // Wait for webhook to populate session with joyfulSessionId
          logger.debug(`[DAEMON RUN] Waiting for session webhook for PID ${joyfulProcess.pid}`);

          return new Promise((resolve) => {
            // Set timeout for webhook
            const timeout = setTimeout(() => {
              pidToAwaiter.delete(joyfulProcess.pid!);
              logger.debug(`[DAEMON RUN] Session webhook timeout for PID ${joyfulProcess.pid}`);
              resolve({
                type: 'error',
                errorMessage: `Session webhook timeout for PID ${joyfulProcess.pid}`
              });
            ...
            // Register awaiter
            pidToAwaiter.set(joyfulProcess.pid!, (completedSession) => {
              clearTimeout(timeout);
              logger.debug(`[DAEMON RUN] Session ${completedSession.joyfulSessionId} fully spawned with webhook`);
              resolve({
                type: 'success',
                sessionId: completedSession.joyfulSessionId!
              });
    ...
    const stopSession = (sessionId: string): boolean => {
      ...
      for (const [pid, session] of pidToTrackedSession.entries()) {
        if (session.joyfulSessionId === sessionId ||
    ...
    const { port: controlPort, stop: stopControlServer } = await startDaemonControlServer({
      ...
      stopSession,
      spawnSession,
      requestShutdown: () => requestShutdown('joyful-cli'),
    ...
    apiMachine.setRPCHandlers({
      spawnSession,
      stopSession,
      requestShutdown: () => requestShutdown('joyful-app')
    });
    ...
    // 4. Write heartbeat
    const heartbeatIntervalMs = parseInt(process.env.JOYFUL_DAEMON_HEARTBEAT_INTERVAL || '60000');
    ...
    // Setup signal handlers
    const cleanupAndShutdown = async (source: 'joyful-app' | 'joyful-cli' | 'os-signal' | 'exception', errorMessage?: string) => {
