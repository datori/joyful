 ...
 * 
 * We created a wrapper script `bin/joyful.mjs` with a shebang `#!/usr/bin/env node`.
 * This allows direct execution on Unix systems and NPM automatically generates 
 * Windows-specific wrapper scripts (`joyful.cmd` and `joyful.ps1`) when it sees 
 ...
 * 
 * **Unix/Linux/macOS:**
 * 1. User runs `joyful` command
 * 2. Shell directly executes `bin/joyful.mjs` (shebang: `#!/usr/bin/env node`)
 * 3. `bin/joyful.mjs` either execs `node --no-warnings --no-deprecation dist/index.mjs` or imports `dist/index.mjs` directly
 * 
 * **Windows:**
 * 1. User runs `joyful` command  
 * 2. NPM wrapper (`joyful.cmd`) calls `node bin/joyful.mjs`
 * 3. `bin/joyful.mjs` either execs `node --no-warnings --no-deprecation dist/index.mjs` or imports `dist/index.mjs` directly
 * 
 * ## The Spawning Problem
 * 
 * When our code needs to spawn Happy cli as a subprocess (for daemon processes), 
 * we were trying to execute `bin/joyful.mjs` directly. This fails on Windows 
 ...
 * 
 * This function bypasses the wrapper script (bin/joyful.mjs) and spawns the 
...
export function spawnHappyCLI(args: string[], options: SpawnOptions = {}): ChildProcess {
  ...
  // bypassing the 'joyful' wrapper that would normally be found in the shell's PATH.
  // However, we log it as 'joyful' here because other engineers are typically looking
