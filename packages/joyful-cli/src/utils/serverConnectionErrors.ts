...
export interface OfflineReconnectionConfig<TSession> {
    /** Server URL to health-check against (e.g., 'https://api.example.com') */
    serverUrl: string;
    ...
    const attemptReconnect = async () => {
        ...
        try {
            ...
            if (axios.isAxiosError(e) && e.response?.status === 401) {
                ...
                config.onNotify('❌ Authentication failed. Please re-authenticate with `joyful auth`.');
...
export const ERROR_DESCRIPTIONS: Record<string, string> = {
    ...
    // HTTP errors
    '401': 'authentication failed - run `joyful auth`',
