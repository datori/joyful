 ...
 * Ensures consistent tmux handling across joyful-cli with proper session naming
 */
...
export class TmuxUtilities {
    ...
    public static readonly DEFAULT_SESSION_NAME = "joyful";
    ...
    async spawnInTmux(
        ...
        try {
            ...
            // - undefined: Use first existing session or create "joyful"
            // - empty string: Use first existing session or create "joyful"
            ...
            if (!sessionName) {
                ...
                if (listResult && listResult.returncode === 0 && listResult.stdout.trim()) {
                    ...
                    // No sessions exist, create "joyful"
                    sessionName = 'joyful';
                    logger.debug(`[TMUX] No existing sessions, using default: ${sessionName}`);
                }
            }

            const windowName = options.windowName || `joyful-${Date.now()}`;
