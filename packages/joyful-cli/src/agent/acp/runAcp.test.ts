...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    mockProjectPath: vi.fn(() => '/tmp/joyful'),
...
vi.mock('@/daemon/run', () => ({
  initialMachineMetadata: { host: 'host', platform: 'darwin', joyfulCliVersion: 'test', homeDir: '/tmp', joyfulHomeDir: '/tmp/.joyful', joyfulLibDir: '/tmp/joyful' },
}));
...
describe('runAcp', () => {
  ...
  it('updates session metadata with ACP config options (models and operating modes)', async () => {
    ...
    const baseMetadata = {
      ...
      host: 'host',
      homeDir: '/home/user',
      joyfulHomeDir: '/home/user/.joyful',
      joyfulLibDir: '/repo/.joyful/lib',
      joyfulToolsDir: '/repo/.joyful/tools',
    };
