...
export interface GeminiBackendOptions extends AgentFactoryOptions {
  ...
  /** OAuth token from Joyful server (via 'joyful connect gemini') - highest priority */
  cloudToken?: string;
...
export function createGeminiBackend(options: GeminiBackendOptions): GeminiBackendResult {
  ...
  // 1. Joyful server OAuth token (via 'joyful connect gemini') - highest priority
  ...
  if (!apiKey) {
    logger.warn(`[Gemini] No API key found. Run 'joyful connect gemini' to authenticate via Google OAuth, or set ${GEMINI_API_KEY_ENV} environment variable.`);
  }
  ...
  const backendOptions: AcpBackendOptions = {
    ...
    hasChangeTitleInstruction: (prompt: string) => {
      ...
      return lower.includes('change_title') ||
             ...
             lower.includes('mcp__joyful__change_title');
    },
  };
