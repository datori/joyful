 ...
 * for joyful CLI including configuration, daemon status, logs, and links
 */
...
export function getEnvironmentInfo(): Record<string, any> {
    return {
        PWD: process.env.PWD,
        JOYFUL_HOME_DIR: process.env.JOYFUL_HOME_DIR,
        JOYFUL_SERVER_URL: process.env.JOYFUL_SERVER_URL,
        JOYFUL_PROJECT_ROOT: process.env.JOYFUL_PROJECT_ROOT,
        DANGEROUSLY_LOG_TO_SERVER_FOR_AI_AUTO_DEBUGGING: process.env.DANGEROUSLY_LOG_TO_SERVER_FOR_AI_AUTO_DEBUGGING,
        NODE_ENV: process.env.NODE_ENV,
        DEBUG: process.env.DEBUG,
        workingDirectory: process.cwd(),
        processArgv: process.argv,
        joyfulDir: configuration?.joyfulHomeDir,
...
export async function runDoctorCommand(filter?: 'all' | 'daemon'): Promise<void> {
    ...
    if (filter === 'all') {
        ...
        const wrapperPath = join(projectRoot, 'bin', 'joyful.mjs');
        ...
        console.log(`Happy Home: ${chalk.blue(configuration.joyfulHomeDir)}`);
        console.log(`Server URL: ${chalk.blue(configuration.serverUrl ?? "(not set)")}`);
        console.log(`Logs Dir: ${chalk.blue(configuration.logsDir)}`);

        // Environment
        console.log(chalk.bold('\n🌍 Environment Variables'));
        const env = getEnvironmentInfo();
        console.log(`JOYFUL_HOME_DIR: ${env.JOYFUL_HOME_DIR ? chalk.green(env.JOYFUL_HOME_DIR) : chalk.gray('not set')}`);
        console.log(`JOYFUL_SERVER_URL: ${env.JOYFUL_SERVER_URL ? chalk.green(env.JOYFUL_SERVER_URL) : chalk.gray('not set')}`);
    ...
    try {
        ...
        if (allProcesses.length > 0) {
            ...
            console.log(chalk.red('❌ No joyful processes found'));
        }

        if (filter === 'all' && allProcesses.length > 1) { // More than just current process
            console.log(chalk.bold('\n💡 Process Management'));
            console.log(chalk.gray('To clean up runaway processes: joyful doctor clean'));
        }
    } catch (error) {
    ...
    if (filter === 'all') {
        ...
        console.log(chalk.bold('\n🐛 Support & Bug Reports'));
        console.log(`Report issues: ${chalk.blue('https://github.com/slopus/joyful/issues')}`);
        console.log(`Documentation: ${chalk.blue('https://github.com/slopus/joyful')}`);
    }
