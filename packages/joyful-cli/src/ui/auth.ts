...
export async function doAuth(): Promise<Credentials | null> {
    ...
    try {
        if (process.env.DEBUG) {
            console.log(`[AUTH DEBUG] Sending auth request to: ${configuration.requireServerUrl()}/v1/auth/request`);
            console.log(`[AUTH DEBUG] Public key: ${encodeBase64(keypair.publicKey).substring(0, 20)}...`);
        }
        await axios.post(`${configuration.requireServerUrl()}/v1/auth/request`, {
...
async function doMobileAuth(keypair: tweetnacl.BoxKeyPair): Promise<Credentials | null> {
    ...
    const authUrl = 'joyful://terminal?' + encodeBase64Url(keypair.publicKey);
...
async function doWebAuth(keypair: tweetnacl.BoxKeyPair): Promise<Credentials | null> {
    ...
    // someone running joyful inside a devcontainer that they saw the
    // "Complete authentication in your browser window." but nothing opened.
    // https://github.com/slopus/joyful/issues/19
...
async function waitForAuthentication(keypair: tweetnacl.BoxKeyPair): Promise<Credentials | null> {
    ...
    try {
        while (!cancelled) {
            try {
                const response = await axios.post(`${configuration.requireServerUrl()}/v1/auth/request`, {
                    publicKey: encodeBase64(keypair.publicKey),
                    supportsV2: true
                });
