 ...
 *     │                                         
 *     ├─► generateHookSettingsFile(port) ──► ~/.joyful/tmp/hooks/session-hook-<pid>.json
 ...
 * 
 * ### Triggered By
 * - `joyful` (fresh start) - new session created
 * - `joyful --continue` - continues last session (may fork)
 * - `joyful --resume` - interactive picker, then resume
 * - `joyful --resume <id>` - resume specific session
