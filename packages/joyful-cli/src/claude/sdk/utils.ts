 ...
 * - JOYFUL_CLAUDE_PATH: Force a specific path to claude executable
 * - JOYFUL_USE_BUNDLED_CLAUDE=1: Force use of node_modules version (skip global search)
 * - JOYFUL_USE_GLOBAL_CLAUDE=1: Force use of global version (if available)
 */
export function getDefaultClaudeCodePath(): string {
    ...
    if (process.env.JOYFUL_CLAUDE_PATH) {
        logger.debug(`[Claude SDK] Using JOYFUL_CLAUDE_PATH: ${process.env.JOYFUL_CLAUDE_PATH}`)
        return process.env.JOYFUL_CLAUDE_PATH
    }

    // Force bundled version if requested
    if (process.env.JOYFUL_USE_BUNDLED_CLAUDE === '1') {
