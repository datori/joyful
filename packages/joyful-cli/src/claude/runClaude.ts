...
export async function runClaude(credentials: Credentials, options: StartOptions = {}): Promise<void> {
    ...
    if (options.startedBy === 'daemon' && options.startingMode === 'local') {
        throw new Error('Daemon-spawned sessions cannot use local/interactive mode. Use --joyful-starting-mode remote or spawn sessions directly from terminal.');
    }
    ...
    let metadata: Metadata = {
        ...
        joyfulToolsDir: resolve(projectPath(), 'tools', 'unpacked'),
    ...
    if (!response) {
        ...
        const reconnection = startOfflineReconnection({
            serverUrl: configuration.requireServerUrl(),
            onReconnected: async () => {
    ...
    logger.debug(`[START] Happy MCP server started at ${joyfulServer.url}`);
    ...
    session.onUserMessage((message) => {
        ...
        if (message.meta?.hasOwnProperty('effortLevel')) {
            ...
            logger.debug(`[loop] Effort level updated from user message: ${messageEffortLevel || 'reset to none'}`);
        } else {
    ...
    const exitCode = await loop({
        ...
        messageQueue,
        api,
        allowedTools: joyfulServer.toolNames.map(toolName => `mcp__joyful__${toolName}`),
