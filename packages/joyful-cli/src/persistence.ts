/**
 * Minimal persistence functions for joyful CLI
 * 
 * Handles settings and private key storage in ~/.joyful/ or local .joyful/
 */
...
// AI backend profile schema - MUST match joyful app exactly
...
// Helper functions matching the joyful app exactly
...
interface Settings {
  ...
  // Profile management settings (synced with joyful app)
  ...
  try {
    ...
    if (schemaVersion > SUPPORTED_SCHEMA_VERSION) {
      logger.warn(
        ...
        'Update joyful-cli for full functionality.'
      );
    }
...
export async function writeSettings(settings: Settings): Promise<void> {
  if (!existsSync(configuration.joyfulHomeDir)) {
    await mkdir(configuration.joyfulHomeDir, { recursive: true })
  }
...
/**
 * Update profiles (synced from joyful app) with validation
 */
