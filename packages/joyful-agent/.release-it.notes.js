...
function getLatestStableTag() {
  const tagsRaw = execSync(`git tag --list "joyful-agent-v*" --sort=-v:refname`, {
    encoding: "utf8",
  });
  ...
  // Only accept stable semver tags like joyful-agent-v1.2.3 (no prerelease suffix).
  const stableTag = tags.find((t) => /^joyful-agent-v\d+\.\d+\.\d+$/.test(t));
...
async function generateReleaseNotes() {
  try {
    ...
    try {
      gitLog = execSync(
        `git log ${commitRange} --pretty=format:"%h - %s (%an, %ar)" --no-merges -- packages/joyful-agent`,
        { encoding: "utf8" }
      );
    } catch (error) {
      console.error(
      ...
      gitLog = execSync(
        `git log -10 --pretty=format:"%h - %s (%an, %ar)" --no-merges -- packages/joyful-agent`,
        { encoding: "utf8" }
      );
    }
...
The release should cover commits since the latest stable joyful-agent tag (joyful-agent-vX.Y.Z): ${
      fromTag ?? "(none)"
    }.
