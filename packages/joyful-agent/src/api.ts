...
import type { SessionMessage as WireSessionMessage } from 'joyful-wire';
...
function handleApiError(err: unknown, context: string): never {
    if (err instanceof AxiosError) {
        ...
        if (status === 401) {
            throw new Error('Authentication expired. Run `joyful-agent auth login` to re-authenticate.');
        }
        if (status === 403) {
