/**
 * Acceptance tests for joyful-agent CLI — Task 10 verification.
 *
 * Verifies:
 ...
 * 4. Interop: sessions with dataKey encryption (joyful-agent) and legacy encryption (joyful-cli)
 */
...
const binPath = resolve(__dirname, '..', 'bin', 'joyful-agent.mjs');
...
function runCli(...args: string[]): { stdout: string; stderr: string; exitCode: number } {
    try {
        const stdout = execFileSync(process.execPath, [
            ...
            binPath,
            ...args,
        ], { encoding: 'utf-8', env: { ...process.env, JOYFUL_HOME_DIR: '/tmp/nonexistent-joyful-acceptance' } });
...
describe('Acceptance: All 8 CLI operations', () => {
    ...
    describe('2. list command', () => {
        ...
        it('fails with auth error when not authenticated', () => {
            ...
            expect(stderr).toContain('joyful-agent auth login');
        });
    });
...
describe('Acceptance: Error handling', () => {
    ...
    describe('server error handling (via API error mapper)', () => {
        it('HTTP 401 maps to re-authenticate message', async () => {
            ...
            const errorMsg = 'Authentication expired. Run `joyful-agent auth login` to re-authenticate.';
            expect(errorMsg).toContain('joyful-agent auth login');
        });
...
describe('Acceptance: Interop — dataKey vs legacy encryption', () => {
    ...
    it('session created with legacy encryption (joyful-cli style) can be decrypted', () => {
        const creds = makeCredentials();
    ...
    it('messages encrypted with dataKey can be round-tripped', () => {
        const sessionKey = getRandomBytes(32);
        const messageContent = { role: 'user', content: { type: 'text', text: 'Hello from joyful-agent' } };
...
describe('Acceptance: Full test suite runs', () => {
    ...
    it('config loads with correct defaults', () => {
        const origUrl = process.env.JOYFUL_SERVER_URL;
        const origHome = process.env.JOYFUL_HOME_DIR;
        ...
        try {
            ...
            expect(config.homeDir).toContain('.joyful');
            expect(config.credentialPath).toContain('agent.key');
        } finally {
            if (origUrl !== undefined) process.env.JOYFUL_SERVER_URL = origUrl;
            if (origHome !== undefined) process.env.JOYFUL_HOME_DIR = origHome;
        }
    });
});
