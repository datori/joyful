...
export function connectRoutes(app: Fastify) {
    ...
    app.get('/v1/connect/github/callback', {
        ...
        if (!tokenData) {
            ...
            return reply.redirect(`${process.env.JOYFUL_WEBAPP_URL || ''}?error=invalid_state`);
        }
        ...
        if (!clientId || !clientSecret) {
            return reply.redirect(`${process.env.JOYFUL_WEBAPP_URL || ''}?error=server_config`);
        }
        ...
        try {
            ...
            if (tokenResponseData.error) {
                return reply.redirect(`${process.env.JOYFUL_WEBAPP_URL || ''}?error=${encodeURIComponent(tokenResponseData.error)}`);
            }
            ...
            if (!userResponse.ok) {
                return reply.redirect(`${process.env.JOYFUL_WEBAPP_URL || ''}?error=github_user_fetch_failed`);
            }
            ...
            return reply.redirect(`${process.env.JOYFUL_WEBAPP_URL || ''}?github=connected&user=${encodeURIComponent(userData.login)}`);

        } catch (error) {
            log({ module: 'github-oauth' }, `Error in GitHub GET callback: ${error}`);
            return reply.redirect(`${process.env.JOYFUL_WEBAPP_URL || ''}?error=server_error`);
        }
    });
